        ...
        <div className="ml-4 border-l border-gray-200 dark:border-gray-700 pl-1">
          ...
          {childNotes.map(note => (
             <div
               ...
               className={`
                 group relative cursor-pointer px-3 py-2 my-1 rounded-md border-l-2 transition-all duration-200
             ...
             >
               <div className="flex items-center gap-2">
                 ...
                 <span className={`truncate text-sm ${activeNoteId === note.id ? 'font-semibold text-primary dark:text-accent1' : 'text-gray-700 dark:text-gray-300'}`}>{note.title || 'Sin título'}</span>
               </div>
               {/* --- VISUALIZACIÓN DE TAGS --- */}
               {note.tags && note.tags.length > 0 && (
                 <div className="flex flex-wrap gap-1 mt-1 ml-5">
                   {note.tags.map((t, idx) => (
                     <span key={idx} className="text-[10px] px-1.5 py-0.5 rounded bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-400">
                       #{t.tag.name}
                     </span>
                   ))}
                 </div>
...
export default function NoteList({ notes, folders, activeNoteId, notebookId, onSelectNote, onCreateNote, onDeleteNote, onRefresh }) {
            ...
            {rootNotes.map(note => (
              ...
              >
                 <div className="flex items-center gap-2">
                    ...
                    <span className={`truncate text-sm ${activeNoteId === note.id ? 'font-semibold text-primary dark:text-white' : 'text-gray-700 dark:text-gray-300'}`}>{note.title || 'Sin título'}</span>
                 </div>
                 ...
                 <button onClick={(e) => { e.stopPropagation(); onDeleteNote(note.id); }} className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-gray-400 opacity-0 group-hover:opacity-100 hover:text-red-500 transition-opacity"><FiTrash size={14} /></button>
              </div>
            ))}
        </div>
